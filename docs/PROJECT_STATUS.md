...
- **Desktop Search Density Polish:** Removed the redundant A-Z sort dropdown from the desktop header (`@media (min-width: 769px) { .sort-select-wrap { display: none !important; } }`), allowing the search bar and action buttons to flow cleanly without top-bar clutter, while preserving sorting in the mobile swipeable rail. Cleaned raw unicode `⌵` characters from `<option>` elements.
