...
def extract_media_technical_specs(path, movie_meta=None):
    ...
    streams = probe.get('streams', []); fmt = probe.get('format', {})
    ...
    for s in streams:
        if s.get('codec_type') == 'video':
            ...
            specs['video_codec'] = codec_map.get(codec, codec.upper() if codec else 'Video'); specs['aspect_ratio'] = s.get('display_aspect_ratio') or ''; break
    for s in streams:
        if s.get('codec_type') == 'audio':
            ...
            specs['audio_codec'] = audio_map.get(acodec, acodec.upper() if acodec else 'Audio'); chan_map = {1: '1.0 Mono', 2: '2.0 Stereo', 6: '5.1 Surround', 8: '7.1 Surround'}; specs['audio_channels'] = chan_map.get(channels, f"{channels} ch" if channels else ''); break
