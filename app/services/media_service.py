...
def movie(path, db, device_id=None):
    """Build movie metadata, using playback progress scoped to the requesting client device."""
    name = path.relative_to(config.MEDIA_ROOT).as_posix()
    meta = db.execute("SELECT * FROM movies WHERE filename=?", (name,)).fetchone()
    progress = None
    if device_id:
        progress = db.execute(
            "SELECT position,duration,updated_at FROM device_progress WHERE device_id=? AND filename=?",
            (device_id, name)
        ).fetchone()
    # Backward-compatible fallback for devices that have never saved device-specific progress.
    if progress is None:
        progress = db.execute("SELECT position,duration,updated_at FROM progress WHERE filename=?", (name,)).fetchone()
...
def get_movies(device_id=None):
    """Retrieve metadata dictionaries for all videos using client-scoped playback progress."""
    if device_id is None:
        device_id = _request_device_id()
    db = get_db()
    try:
        return [movie(p, db, device_id=device_id) for p in video_paths()]
    finally:
        db.close()
...
def extract_media_technical_specs(path, movie_meta=None):
    ...
    specs = {
        'resolution': '', 'resolution_badge': '', 'video_codec': '', 'video_profile': '',
        ...
        'bitrate': '', 'subtitles': [], 'aspect_ratio': ''
    }
    ...
    if bit_rate:
        try:
            br_num = int(bit_rate)
            specs['bitrate'] = f"{br_num / 1_000_000:.1f} Mbps" if br_num >= 1_000_000 else f"{br_num // 1000} kbps"
    ...
    for s in streams:
        if s.get('codec_type') == 'video':
            ...
            codec_map = {'hevc': 'HEVC (H.265)', 'h264': 'H.264 (AVC)', 'vp9': 'VP9', 'av01': 'AV1', 'av1': 'AV1', 'mpeg4': 'MPEG-4', 'vc1': 'VC-1'}
    ...
    for s in streams:
        if s.get('codec_type') == 'audio':
            acodec = (s.get('codec_name') or '').lower(); channels = s.get('channels') or 0
            audio_map = {'eac3': 'Dolby Digital Plus', 'ac3': 'Dolby Digital', 'truehd': 'Dolby TrueHD', 'dts': 'DTS', 'dts-hd': 'DTS-HD', 'aac': 'AAC', 'flac': 'FLAC', 'mp3': 'MP3', 'opus': 'Opus', 'vorbis': 'Vorbis'}
            ...
            break
    try:
        specs['subtitles'] = [t['label'] for t in tracks(path, movie_meta)]
    except Exception:
        specs['subtitles'] = []
    return specs
...
def purge_media(filename):
    ...
    tmdb_id = value(movie_row, 'tmdb_id'); poster_path = value(movie_row, 'poster_path')
    ...
    db.execute("DELETE FROM device_progress WHERE filename=?", (rel_filename,))
    db.execute("DELETE FROM device_watch_history WHERE filename=?", (rel_filename,))
    db.commit(); db.close()
    file_deleted = False
    if path.is_file(): path.unlink(missing_ok=True); file_deleted = True
    path.with_name(f".upload_{path.name}.part").unlink(missing_ok=True)
...
def get_managed_media_items():
    ...
    items = []
    for m in all_movies:
        p = m['path']; rel_fn = m['filename']; file_exists = p.is_file() if isinstance(p, Path) else Path(p).is_file(); st = p.stat() if file_exists else None
        size_bytes = st.st_size if st else 0; size_str = format_bytes_display(size_bytes) if size_bytes else "—"; suffix = Path(rel_fn).suffix.upper().lstrip('.'); is_mkv = needs_transcode(p)
        active_t = active_map.get(rel_fn); hls_cached = False
        if is_mkv and file_exists:
            hls_dir = hls_cache_dir(p); pl = hls_dir / 'playlist.m3u8'; hls_cached = _is_hls_truly_complete(pl, p)
        ...
        if m.get('poster'):
            try:
                poster_url = url_for('poster', filename=m['poster'][6:]) if m['poster'].startswith('local:') else url_for('tmdb_poster', tmdb_id=m['poster'][5:])
            except Exception: poster_url = None
        items.append({'filename': rel_fn, 'title': m['title'], 'year': m['year'], 'size_bytes': size_bytes, 'size_str': size_str, 'poster_url': poster_url, 'stream_status': stream_status, 'position': m['position'], 'duration': m['duration'], 'percent': m['percent'], 'path': p})
    return items
