...
from app.services.media_service import extract_media_technical_specs, get_movies, movie
...
def home():
    ...
    try: telemetry = get_system_telemetry()
    except Exception: telemetry = None
    return render_template('library.html', movies=movies, watching=watching, q=q, sort=sort, active_transcodes=active_transcodes, telemetry=telemetry)
...
def details(filename):
    ...
    if m.get('details_json'):
        try: extended = json.loads(m['details_json'])
        except Exception: extended = {}
    specs = extract_media_technical_specs(path, m); formatted_runtime = format_runtime_display(m.get('runtime')); active_transcodes = get_active_transcodes(); transcode_info = next((t for t in active_transcodes if t['filename'] == filename), None)
    return render_template('details.html', movie=m, backdrop=backdrop, extended=extended, specs=specs, formatted_runtime=formatted_runtime, transcode_info=transcode_info)
...
def watch(filename):
    ...
    path = safe_path(filename)
    if not is_video(path): abort(404)
    db = get_db(); m = movie(path, db); db.close(); active_transcodes = get_active_transcodes(); transcode_info = next((t for t in active_transcodes if t['filename'] == filename), None)
    html = render_template('player.html', movie=m, tracks=tracks(path, m), transcode_info=transcode_info)
...
def manifest():
    ...
    manifest_data = {'name': "Anis' Media Server", 'short_name': 'Anis Media', 'start_url': '/', 'display': 'standalone', 'background_color': '#090b10', 'theme_color': '#090b10', 'icons': [{'src': '/icon.svg', 'sizes': 'any', 'type': 'image/svg+xml'}]}
...
def icon():
    ...
    svg = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><defs><linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#ff334b"/><stop offset="100%" stop-color="#b80017"/></linearGradient></defs><rect width="128" height="128" rx="28" fill="url(#g)"/><rect x="2" y="2" width="124" height="124" rx="26" fill="none" stroke="rgba(255,255,255,0.25)" stroke-width="2"/><path fill="#ffffff" d="M50 38l38 26-38 26V38z"/><circle cx="84" cy="42" r="7" fill="#ffffff" fill-opacity="0.9"/></svg>'''
...
def manage():
    ...
    items = get_managed_media_items(); total_size = sum(i['size_bytes'] for i in items); cache_dir = get_cache_dir(); hls_dir = cache_dir / 'hls'; transcode_dir = cache_dir / 'transcodes'; hls_size = sum(f.stat().st_size for f in hls_dir.rglob('*') if f.is_file()) if hls_dir.is_dir() else 0; mp4_size = sum(f.stat().st_size for f in transcode_dir.rglob('*') if f.is_file()) if transcode_dir.is_dir() else 0
    return render_template('manage.html', items=items, total_movies=len(items), total_size_str=format_bytes_display(total_size), total_cache_str=format_bytes_display(hls_size + mp4_size), active_transcodes=get_active_transcodes())
...
def devices():
    ...
    current_dev_id, _ = get_or_create_device_id(request); dev_list, stats = get_all_devices(current_device_id=current_dev_id)
    return render_template('devices.html', devices=dev_list, stats=stats, current_dev_id=current_dev_id)
    ...
    try:
        if response.mimetype == 'text/html':
            ...
            if is_new or not request.cookies.get('ms_device_id'):
                response.set_cookie('ms_device_id', dev_id, max_age=31536000, path='/', samesite='Lax')
