...
- **Multi-Chunk HLS Discontinuity Alignment & Non-Destructive Seek Recovery:** Multi-chunk transcode scheduler enforces timeline timestamp continuity by removing `-avoid_negative_ts make_zero`, injecting RFC 8216 `#EXT-X-DISCONTINUITY` boundaries in `_update_master_playlist()`, and automatically reconciling existing master playlists on disk (`reconcile_hls_playlist_discontinuities()`). Player error fallback preserves seek target timestamps, preventing playback resets to `0:00` when forward seeking across chunk boundaries.
- **Unified Navigation Header & Brand Identity:** Redesigned frosted obsidian glass header with 3D glossy play SVG brand icon, two-tone typography (**Anis'** + **Media Library**), subtitle (**PLAY • ORGANIZE • ENJOY**), modular SVG pill buttons, and responsive horizontal swipeable action rails across all 5 pages (`/`, `/movie/<filename>`, `/player/<filename>`, `/manage`, `/devices`).
